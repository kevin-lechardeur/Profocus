...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${DateFormat('EEEE').format(today)}'),
        centerTitle: true,
      ),
      body: ListView.builder(
        itemCount: 18, // Une ligne par heure
        itemBuilder: (context, index) {
          final hour = DateTime(today.year, today.month, today.day, index + 6);
          final eventsForThisHour = _controller.getEventsForHour(hour);

          if (eventsForThisHour.isEmpty || !eventsForThisHour.any((event) =>
              event.startTime.isAtSameMomentAs(hour))) {
            return SizedBox.shrink(); // Évite d’afficher la section
          }
          return Column(
